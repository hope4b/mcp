...
## Next Priority Queue
1. Review/QA `MCP-ENTRYPOINTS-001` agent-routing implementation. Implementation report is `docs/agents/tasks/2026-06-14-mcp-agent-entrypoints-tool-contract-implementation-result.md`. Local checks passed via targeted `unittest` (`13 tests`), full `unittest` discovery (`50 tests`), temp-cache `compileall`, and `git diff --check`; `pytest` is unavailable in the active interpreter. No runtime/preprod checks, commit, push, deploy, or Onto object actions were performed.
